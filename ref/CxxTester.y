...
%type <expression> delete_expression direct_abstract_declarator direct_abstract_declarator.opt direct_new_declarator
...
%type <expression> multiplicative_expression new_declarator new_expression new_type_id
...
%type <name> enum_specifier enumerator global_scope id id_scope identifier linkage_specification mem_initializer_id
%type <name> namespace_definition nested_id nested_pseudo_destructor_id nested_special_function_id
%type <name> operator operator_function_id pseudo_destructor_id scoped_id scoped_pseudo_destructor_id scoped_special_function_id
...
global_scope:                       SCOPE                                                       { $$ = 0; }
    |                               TEMPLATE global_scope                                       { $$ = YACC_SET_TEMPLATE_SCOPE($2); }
...
scoped_id:                          nested_id
    |                               global_scope nested_id                                      { $$ = YACC_SCOPED_ID($1, $2); }
...
scoped_special_function_id:         nested_special_function_id
    |                               global_scope nested_special_function_id                     { $$ = YACC_SCOPED_ID($1, $2); }
...
scoped_pseudo_destructor_id:        nested_pseudo_destructor_id
    |                               global_scope scoped_pseudo_destructor_id                    { $$ = YACC_SCOPED_ID($1, $2); }
 ...
 */
string:                             StringLiteral                                               { $$ = YACC_STRINGS($1, 0); }
...
abstract_expression:                parenthesis_clause                                          { $$ = YACC_ABSTRACT_FUNCTION_EXPRESSION($1); }
    ...
    |                               TEMPLATE abstract_expression                                { $$ = YACC_SET_TEMPLATE_EXPRESSION($2); }
...
/*  |                               SIZEOF '(' type_id ')'                                  -- covered by unary_expression */
    |                               new_expression
    |                               global_scope new_expression                             { $$ = YACC_GLOBAL_EXPRESSION($2); }
    |                               delete_expression
    |                               global_scope delete_expression                          { $$ = YACC_GLOBAL_EXPRESSION($2); }
...
control_statement:                  labeled_statement
    ...
    |                               iteration_statement
    |                               jump_statement
labeled_statement:                  identifier ':' looping_statement                            { $$ = YACC_LABEL_STATEMENT($1, $3); }
...
ptr_operator_seq:                   ptr_operator                                                { $$ = YACC_POINTER_EXPRESSION($1, 0); }
...
abstract_declarator.opt:            /* empty */                                                 { $$ = 0; }
    |                               ptr_operator abstract_declarator.opt                        { $$ = YACC_POINTER_EXPRESSION($1, $2); }
    |                               direct_abstract_declarator
direct_abstract_declarator.opt:     /* empty */                                                 { $$ = 0; }
    |                               direct_abstract_declarator
...
abstract_parameter_declaration:     abstract_pointer_declaration
    |                               and_expression '&'                                          { $$ = YACC_LOGICAL_AND_EXPRESSION($1, 0); }
    |                               and_expression '&' abstract_pointer_declaration             { $$ = YACC_LOGICAL_AND_EXPRESSION($1, $3); }
...
templated_abstract_declaration:     abstract_pointer_declaration
    |                               templated_and_expression '&'                                { $$ = YACC_LOGICAL_AND_EXPRESSION($1, 0); }
    |                               templated_and_expression '&' abstract_pointer_declaration   { $$ = YACC_LOGICAL_AND_EXPRESSION($1, $3); }
...
ctor_definition:                    constructor_head function_try_block                         { $$ = YACC_FUNCTION_DEFINITION($1, $2); }
    |                               constructor_head function_body                              { $$ = YACC_FUNCTION_DEFINITION($1, $2); }
...
accessibility_specifier:            access_specifier ':'                                        { $$ = YACC_ACCESSIBILITY_DECLARATION($1); }
...
template_parameter:                 simple_type_parameter                                       { $$ = YACC_TYPE_TEMPLATE_PARAMETER($1, 0); }
    |                               simple_type_parameter '=' type_id                           { $$ = YACC_TYPE_TEMPLATE_PARAMETER($1, $3); }
    |                               templated_type_parameter                                    { $$ = YACC_TEMPLATED_TEMPLATE_PARAMETER($1, 0); }
    |                               templated_type_parameter '=' identifier                     { $$ = YACC_TEMPLATED_TEMPLATE_PARAMETER($1, $3); }
    |                               templated_parameter_declaration                             { $$ = YACC_TEMPLATE_PARAMETER($1); }
    |                               bang error                                                  { $$ = 0; YACC_UNBANG($1, "Bad template-parameter."); }
simple_type_parameter:              CLASS                                                       { $$ = YACC_CLASS_TEMPLATE_PARAMETER(0); }
/*  |                               CLASS identifier                                            -- covered by parameter_declaration */
    |                               TYPENAME                                                    { $$ = YACC_TYPENAME_TEMPLATE_PARAMETER(0); }
